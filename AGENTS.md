...
**Poda automática de manifestos já desfeitos (`history.Prune`), desde a v0.8.1 — sem isso, o diretório de histórico e `undo --list` cresciam para sempre.** Nada nunca removia um manifesto do disco: `MarkUndone` só grava `UndoneAt`, e `List()` lê todo `.yaml` do diretório sem limite nenhum — uma execução real de `organize-pdf` por semana significa um arquivo novo por semana, indefinidamente, mesmo muito tempo depois de o usuário já ter desfeito (ou decidido nunca desfazer) aquela operação. `Prune(now time.Time, retention time.Duration) ([]string, error)` remove do disco os manifestos com `UndoneAt != nil` cuja idade (`now.Sub(*UndoneAt)`) já passou de `retention` (constante `PruneRetention`, 30 dias); devolve os IDs removidos. `Save()` chama `Prune(time.Now(), PruneRetention)` no fim de toda gravação bem-sucedida, ignorando o erro (mesmo princípio da Decisão 15/relatório: uma tarefa de manutenção secundária não pode fazer uma gravação que já aconteceu parecer que falhou) — a próxima chamada a `Save` tenta de novo, então um erro pontual de poda não acumula lixo indefinidamente. **Regra que não pode ser relaxada: um manifesto pendente (`UndoneAt == nil`) nunca é removido automaticamente, não importa a idade** — é exatamente ele que permite desfazer aquela operação mais tarde; removê-lo sem o usuário pedir tiraria essa capacidade em silêncio, o oposto do que a feature de desfazer promete. Não existe hoje um comando para o usuário podar manifestos pendentes antigos manualmente — se for adicionado, deve exigir confirmação explícita, ao contrário da poda automática (que só toca em manifestos cujo propósito já foi cumprido).
